...
export async function GET(
  ...
  if (detail && detail.id !== id) {
    return NextResponse.json({ ...detail, id, name: `Project ${id}`, progress: 0, budgetUsed: 0, milestones: [], tasks: [], documents: [], risks: [], changeRequests: [], comments: [], timeline: [] });
  }
...
export async function PATCH(
  ...
  const { action, taskId, taskStatus, commentContent, authorName, crTitle, crDescription, crImpact, riskId, riskStatus } = body;

  // Simulate workspace actions
  return NextResponse.json({
    ...
    task: taskId ? { id: taskId, status: taskStatus } : undefined,
    ...
    changeRequest: crTitle ? { id: `cr-${Date.now()}`, title: crTitle, description: crDescription, impact: crImpact, status: "PENDING", requestedBy: authorName || "Rajesh Sharma", createdAt: new Date().toISOString() } : undefined,
    risk: riskId ? { id: riskId, status: riskStatus } : undefined
  });
}
