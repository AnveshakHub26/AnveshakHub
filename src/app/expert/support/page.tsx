  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-[#211F1D]">Expert Support & Knowledge Helpdesk</h1>
          <p className="text-xs text-[#78716A] mt-0.5">Search FAQs, submit technical tickets & track resolution status with AnveshakHub support engineers</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setTicketModal(true)} className="h-8 px-4 bg-[#FF5A36] text-white rounded-lg text-xs font-bold hover:bg-[#E04826] flex items-center gap-1.5">
      ...
      <div className="card-flat rounded-2xl p-6 space-y-4">
        <h3 className="text-xs font-bold text-[#211F1D] uppercase tracking-wide">Search Knowledge Base & FAQs</h3>
        <div className="relative">
          ...
          <input
            ...
            className="pl-10 pr-4 h-10 w-full text-xs border border-[#E2DCD2] rounded-xl focus:outline-none focus:border-[#FF5A36]"
          />
        </div>

        <div className="space-y-3 pt-2">
          {faqs.map(faq => (
            <div key={faq.id} className="border border-[#E2DCD2] rounded-xl p-4 bg-[#FBF7F0] space-y-1.5">
              <span className="text-[8px] font-extrabold px-1.5 py-0.5 rounded bg-[#FFF0ED] text-[#FF5A36] uppercase">{faq.category}</span>
              <h4 className="text-xs font-bold text-[#211F1D]">{faq.question}</h4>
              <p className="text-xs text-[#57534E] font-medium leading-relaxed">{faq.answer}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Support Tickets */}
      <div className="card-flat rounded-2xl p-6 space-y-4">
        <h3 className="text-xs font-bold text-[#211F1D] uppercase tracking-wide">Your Active Support Tickets</h3>
        <div className="space-y-2">
          {tickets.map(tkt => (
            <div key={tkt.id} className="border border-[#E2DCD2] rounded-xl p-4 flex items-center justify-between bg-[#FBF7F0]">
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  <span className="text-[8px] font-mono font-extrabold px-1.5 py-0.5 rounded bg-[#D8D2C7] text-[#211F1D]">{tkt.ticketNumber}</span>
                  <h4 className="text-xs font-bold text-[#211F1D]">{tkt.subject}</h4>
                </div>
                <p className="text-[10px] text-[#A8A196] font-semibold">{tkt.category} · Created {new Date(tkt.createdAt).toLocaleDateString("en-IN")}</p>
              </div>

              <div className="flex items-center gap-3">
                <span className={`text-[8px] font-bold px-2 py-0.5 rounded ${
                  tkt.status === "RESOLVED" ? "bg-[#E8F2EC] text-[#2F6B4F]" : "bg-[#FEF3C7] text-[#B45309]"
                }`}>{tkt.status}</span>
                <Link href={`/expert/support/${tkt.id}`} className="h-7 px-3 border border-[#E2DCD2] hover:border-[#FF5A36] text-[#57534E] hover:text-[#FF5A36] rounded-lg text-[10px] font-bold flex items-center gap-1">
            ...
            >
              <div className="flex justify-between items-center border-b border-[#E2DCD2] pb-3 mb-4">
                <h3 className="text-sm font-bold text-[#211F1D] flex items-center gap-1.5">
                  <MessageSquare className="h-4 w-4 text-[#FF5A36]" /> Create Support Ticket
                </h3>
                <button onClick={() => setTicketModal(false)}><X className="h-4 w-4 text-[#A8A196] hover:text-[#57534E]" /></button>
              </div>

              <div className="space-y-3 text-xs">
                <div>
                  <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Subject *</label>
              ...
              <div className="flex justify-end gap-2 border-t border-[#E2DCD2] pt-4 mt-4">
                <button onClick={() => setTicketModal(false)} className="h-8 px-3 border border-[#E2DCD2] text-[#78716A] rounded-lg text-xs font-semibold hover:bg-[#EFE9DF]">Cancel</button>
