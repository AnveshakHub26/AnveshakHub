    ...
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-[#E2DCD2] pb-4">
        <div className="flex items-center gap-3">
          <Link href="/expert/students" className="h-8 w-8 rounded-lg border border-[#E2DCD2] flex items-center justify-center text-[#78716A] hover:text-[#211F1D] hover:bg-[#FBF7F0]">
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[8px] font-mono font-extrabold px-1.5 py-0.5 rounded bg-[#EFE9DF] text-[#211F1D]">{student.usn}</span>
              <h1 className="text-base font-bold text-[#211F1D]">{student.name}</h1>
            </div>
            <p className="text-xs text-[#78716A] font-semibold">{student.institution} · {student.degree} (Sem {student.semester})</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setEvalModalOpen(true)} className="h-8 px-3 border border-[#E2DCD2] text-[#211F1D] rounded-lg text-xs font-bold hover:bg-[#FBF7F0] flex items-center gap-1">
      ...
      <div className="bg-white border border-[#E2DCD2] rounded-2xl p-6 min-h-[360px]">
        ...
        {activeTab === "skills" && (
          <div className="space-y-6 text-xs">
            <div className="grid grid-cols-3 gap-4 bg-[#FBF7F0] border border-[#E2DCD2] rounded-2xl p-4 text-center">
              <div>
                <span className="text-[9px] text-[#A8A196] font-bold uppercase block">Technical Score</span>
                <span className="text-base font-extrabold text-[#211F1D]">★ {student.technicalSkillScore} / 5</span>
              </div>
              <div>
                <span className="text-[9px] text-[#A8A196] font-bold uppercase block">Soft Skills Score</span>
                <span className="text-base font-extrabold text-[#211F1D]">★ {student.softSkillScore} / 5</span>
              </div>
              <div>
                <span className="text-[9px] text-[#A8A196] font-bold uppercase block">Overall Rating</span>
            ...
            <div className="space-y-3 border-t border-[#E2DCD2] pt-4">
              <h3 className="font-bold text-[#211F1D] uppercase tracking-wide text-[10px]">Expert Evaluation History</h3>
              <div className="space-y-3">
                {student.evaluations.map((ev, i) => (
                  <div key={i} className="border border-[#E2DCD2] rounded-xl p-4 bg-[#FBF7F0] space-y-1">
                    <div className="flex justify-between items-center text-[10px] font-bold text-[#211F1D]">
                      <span>Rating: ★ {ev.overallScore}</span>
                      <span className="text-[#A8A196] font-semibold">{new Date(ev.date).toLocaleDateString("en-IN")}</span>
                    </div>
                    <p className="text-[#57534E] font-medium leading-relaxed">{ev.feedback}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === "goals" && (
          <div className="space-y-3 text-xs">
            <h3 className="font-bold text-[#211F1D] uppercase tracking-wide text-[10px]">Learning Goals & Milestones</h3>
            <div className="space-y-2">
              {student.learningPlan.map(g => (
                <div key={g.id} className="border border-[#E2DCD2] rounded-xl p-3.5 flex items-center justify-between bg-[#FBF7F0]">
                  <div className="flex items-center gap-3">
                    <div className={`h-5 w-5 rounded border flex items-center justify-center ${
                      g.status === "COMPLETED" ? "bg-[#E8F2EC]0 text-white border-green-500" : "border-slate-300 bg-white"
                    }`}>
                      {g.status === "COMPLETED" && <Check className="h-3.5 w-3.5" />}
                    </div>
                    <span className={`font-bold ${g.status === "COMPLETED" ? "line-through text-[#A8A196]" : "text-[#211F1D]"}`}>{g.title}</span>
                  </div>
                  <span className="text-[9px] font-bold text-[#A8A196]">Target: {new Date(g.targetDate).toLocaleDateString("en-IN")}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4 text-xs">
            {student.recommendation ? (
              <div className="border border-purple-100 rounded-2xl p-6 bg-purple-50/40 space-y-3">
                ...
                <pre className="text-[#211F1D] font-sans leading-relaxed whitespace-pre-wrap">{student.recommendation.content}</pre>
              </div>
            ) : (
              <div className="text-center py-12 space-y-3">
                <FileCheck className="h-10 w-10 text-[#C4BCB0] mx-auto" />
                <p className="font-bold text-[#211F1D]">No Recommendation Letter Issued Yet</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setEvalModalOpen(false)}>
            ...
            >
              <div className="flex justify-between items-center border-b border-[#E2DCD2] pb-3 mb-4">
                ...
                <button onClick={() => setEvalModalOpen(false)}><X className="h-4 w-4 text-[#A8A196] hover:text-[#57534E]" /></button>
              </div>

              <div className="space-y-3 text-xs">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[9px] font-bold text-[#78716A] uppercase block mb-1">Technical Skill Score (1-5)</label>
                    <input type="number" step="0.1" value={techScore} onChange={e => setTechScore(e.target.value)}
                      className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg focus:outline-none focus:border-primary text-xs font-bold" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-[#78716A] uppercase block mb-1">Soft Skills Score (1-5)</label>
                    <input type="number" step="0.1" value={softScore} onChange={e => setSoftScore(e.target.value)}
                      className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg focus:outline-none focus:border-primary text-xs font-bold" />
                  </div>
                </div>
                <div>
                  <label className="text-[9px] font-bold text-[#78716A] uppercase block mb-1">Detailed Feedback Notes *</label>
              ...
              <div className="flex justify-end gap-2 border-t border-[#E2DCD2] pt-4 mt-4">
                <button onClick={() => setEvalModalOpen(false)} className="h-8 px-3 border border-[#E2DCD2] text-[#78716A] rounded-lg text-xs font-semibold hover:bg-[#FBF7F0]">Cancel</button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setRecModalOpen(false)}>
            ...
            >
              <div className="flex justify-between items-center border-b border-[#E2DCD2] pb-3 mb-4">
                ...
                <button onClick={() => setRecModalOpen(false)}><X className="h-4 w-4 text-[#A8A196] hover:text-[#57534E]" /></button>
              </div>

              <div className="space-y-3 text-xs">
                <div>
                  <label className="text-[9px] font-bold text-[#78716A] uppercase block mb-1">Recommendation Title *</label>
                ...
                <div>
                  <label className="text-[9px] font-bold text-[#78716A] uppercase block mb-1">Letter Content *</label>
              ...
              <div className="flex justify-end gap-2 border-t border-[#E2DCD2] pt-4 mt-4">
                <button onClick={() => setRecModalOpen(false)} className="h-8 px-3 border border-[#E2DCD2] text-[#78716A] rounded-lg text-xs font-semibold hover:bg-[#FBF7F0]">Cancel</button>
