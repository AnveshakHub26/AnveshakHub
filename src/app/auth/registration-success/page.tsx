...
function StatusBadge({ status }: { status: "completed" | "active" | "pending" }) {
  ...
  if (status === "active") {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#FFF0ED] text-[#FF5A36] border border-[#FFCFC4]">
        <span className="h-1.5 w-1.5 rounded-full bg-[#FF5A36] animate-pulse" /> In Progress
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#EFE9DF] text-[#78716A] border border-[#E2DCD2]">
      ...
      <div className="flex flex-col items-center">
        <div className={[
          ...
          stage.status === "completed"
            ...
            : stage.status === "active"
            ? "bg-[#FF5A36] border-[#FF5A36] text-white ring-4 ring-[#FF5A36]/15"
        ...
        {stage.id < 6 && (
          <div className={[
            ...
            stage.status === "completed" ? "bg-[#BBD9C8]" : "bg-slate-150"
      ...
      <div className="pb-7 flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-1">
          <span className={[
            ...
            stage.status === "active"    ? "text-[#FF5A36]" :
                                           "text-[#78716A]"
          ].join(" ")}>{stage.label}</span>
          <StatusBadge status={stage.status} />
        </div>
        <p className="text-[11px] text-[#78716A] leading-relaxed">{stage.description}</p>
        <div className="mt-1.5 flex items-center gap-1">
          <Clock className="h-3 w-3 text-[#A8A196]" />
          <span className="text-[10px] font-bold text-[#A8A196]">{stage.duration}</span>
        </div>
      </div>
    </motion.div>
  );
    ...
    >
      ...
      >
        ...
        <div className="shrink-0">
          {open ? (
            <ChevronUp className="h-4 w-4 text-[#A8A196]" />
          ) : (
            <ChevronDown className="h-4 w-4 text-[#A8A196]" />
          )}
        </div>
      </button>
            ...
            <div className="px-6 py-5 border-b border-[#E2DCD2] flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="h-9 w-9 rounded-xl bg-[#FFF0ED] border border-blue-100 flex items-center justify-center">
                  <MessageSquare className="h-[1.125rem] w-[1.125rem] text-[#FF5A36]" />
                </div>
                <div>
                  <Dialog.Title className="text-sm font-extrabold text-[#211F1D]">Contact Support</Dialog.Title>
                  <p id="support-modal-desc" className="text-[10px] text-[#A8A196] mt-0.5">Enterprise Verification Support Desk</p>
                </div>
              </div>
            ...
            <div className="p-6">
              {!sent ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full h-9 px-3 rounded-lg border border-[#E2DCD2] text-xs font-medium text-[#211F1D] placeholder-slate-400 outline-none focus:border-[#FF5A36] focus:ring-3 focus:ring-[#FF5A36]/20 transition-all"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-[#78716A] uppercase tracking-wider mb-1.5">Email Address <span className="text-red-500">*</span></label>
                      <input
                        required
                        type="email"
                        value={form.email}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full h-9 px-3 rounded-lg border border-[#E2DCD2] text-xs font-medium text-[#211F1D] outline-none focus:border-[#FF5A36] focus:ring-3 focus:ring-[#FF5A36]/20 transition-all bg-[#FBF7F0]"
                    >
                  ...
                  <div className="flex gap-3 justify-end pt-1">
                    <Dialog.Close asChild>
                      <button type="button" className="h-9 px-4 border border-[#E2DCD2] rounded-lg text-xs font-bold text-[#57534E] hover:bg-[#FBF7F0] transition-colors">
                        Cancel
                      </button>
                    </Dialog.Close>
                    <button
                ...
                <div className="text-center py-8">
                  ...
                  <Dialog.Close asChild>
                    <button className="mt-5 h-9 px-5 bg-[#FF5A36] hover:bg-[#E04826] text-white rounded-lg text-xs font-bold transition-colors">
                      Done
                    </button>
                  </Dialog.Close>
                </div>
...
export default function RegistrationSuccessPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FBF7F0] relative overflow-hidden">
      ...
      <div className="absolute top-1/3 -left-32 w-[400px] h-[400px] bg-[#FF5A36]/4 rounded-full blur-3xl pointer-events-none" />

      {/* ── Header ── */}
      <header className="sticky top-0 z-50 bg-[#FBF7F0]/85 backdrop-blur-md border-b border-[#E2DCD2]/80 py-3 px-4 sm:px-6 lg:px-8">
      ...
      <main className="flex-grow py-10 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-[#FBF7F0] border border-[#E2DCD2] rounded-full px-4 py-1.5 shadow-[var(--shadow-sm)] flex-wrap justify-center">
              {[
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <motion.div
                ...
                className="card-flat rounded-2xl shadow-lg overflow-hidden"
              >
                {/* Top banner */}
                <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 px-8 py-5 flex items-center gap-4">
                  <div className="h-10 w-10 rounded-xl bg-[#FBF7F0]/20 flex items-center justify-center shrink-0">
                    <CheckCircle2 className="h-6 w-6 text-white" />
                  </div>
                  <div>
                    <h1 className="text-base font-black text-white tracking-tight">Registration Submitted Successfully</h1>
                    <p className="text-[11px] text-[#2F6B4F]/100 mt-0.5 font-medium">
                      {summary?.submittedAt ? `Submitted on ${summary.submittedAt}` : "Submission complete"}
                    </p>
                  </div>
                  <div className="ml-auto hidden sm:block">
                    <span className="inline-flex items-center gap-1.5 bg-[#FBF7F0]/20 border border-white/30 px-3 py-1.5 rounded-full text-[10px] font-bold text-white">
                      <span className="h-1.5 w-1.5 rounded-full bg-[#FBF7F0] animate-pulse" />
                ...
                <div className="p-8">
                  <div className="flex flex-col md:flex-row gap-8 items-center">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-extrabold text-[#211F1D] tracking-tight mb-2">
                        {summary?.orgName ? `Thank you, ${summary.orgName}` : "Thank you for registering."}
                      </p>
                      <p className="text-xs text-[#78716A] leading-relaxed mb-5">
                      ...
                      <div className="space-y-2">
                        {[
                          ...
                          <div key={item} className="flex items-center gap-2.5">
                            <div className="h-5 w-5 rounded-full bg-[#E8F2EC] border border-[#BBD9C8] flex items-center justify-center shrink-0">
                              <CheckCircle2 className="h-3 w-3 text-[#2F6B4F]" />
                            </div>
                            <span className="text-[11px] font-semibold text-[#211F1D]">{item}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="px-7 py-5 border-b border-[#E2DCD2] flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="h-8 w-8 rounded-lg bg-[#FFF0ED] border border-blue-100 flex items-center justify-center">
                      <ClipboardCheck className="h-4 w-4 text-[#FF5A36]" />
                    </div>
                    <div>
                      <h2 className="text-sm font-extrabold text-[#211F1D]">Verification Timeline</h2>
                      <p className="text-[10px] text-[#A8A196] mt-0.5">Your registration journey — stage by stage</p>
                    </div>
                  </div>
                  <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-bold text-[#A8A196]">
              ...
              <motion.div
                ...
                className="card-flat rounded-2xl p-7"
              >
                <div className="flex items-center gap-2.5 mb-5 pb-4 border-b border-[#E2DCD2]">
                  ...
                  <div>
                    <h2 className="text-sm font-extrabold text-[#211F1D]">What Happens Next?</h2>
                    <p className="text-[10px] text-[#A8A196] mt-0.5">No action required unless contacted by our team</p>
                  </div>
                </div>
                <div className="grid sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={title} className="flex gap-3 p-4 bg-[#FBF7F0] border border-[#E2DCD2] rounded-xl">
                      <div className="h-8 w-8 rounded-lg bg-[#FBF7F0] border border-[#E2DCD2] flex items-center justify-center shrink-0 shadow-[var(--shadow-sm)]">
                        <Icon className="h-4 w-4 text-[#FF5A36]" />
                      </div>
                      <div>
                        <p className="text-xs font-bold text-[#211F1D]">{title}</p>
                        <p className="text-[10px] text-[#78716A] mt-1 leading-relaxed">{desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                ...
                >
                  <div className="flex items-center gap-2 mb-4 pb-3 border-b border-[#E2DCD2]">
                    ...
                    <h3 className="text-xs font-extrabold text-[#211F1D]">Estimated Timelines</h3>
                  </div>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={label} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                        <span className="text-[11px] text-[#57534E] font-medium">{label}</span>
                        <span className="text-[10px] font-bold text-[#FF5A36] bg-[#FFF0ED] border border-blue-100 px-2 py-0.5 rounded-full">{time}</span>
                      </div>
                    ))}
                  </div>
                  <p className="mt-3 text-[10px] text-[#A8A196] leading-relaxed">
                ...
                >
                  <div className="flex items-center gap-2 mb-4 pb-3 border-b border-[#E2DCD2]">
                    ...
                    <h3 className="text-xs font-extrabold text-[#211F1D]">Notification Channels</h3>
                  </div>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={label} className="flex items-center justify-between">
                        ...
                        <span className={[
                          ...
                          color === "emerald" ? "bg-[#E8F2EC] text-[#2F6B4F] border-[#BBD9C8]" :
                          color === "amber"   ? "bg-[#FEF3C7] text-[#B45309] border-amber-200" :
              ...
              <motion.div
                ...
                className="card-flat rounded-2xl overflow-hidden"
              >
                <div className="px-7 py-5 border-b border-[#E2DCD2] flex items-center gap-2.5">
                  <div className="h-8 w-8 rounded-lg bg-[#EFE9DF] flex items-center justify-center">
                    <HelpCircle className="h-4 w-4 text-[#57534E]" />
                  </div>
                  <div>
                    <h2 className="text-sm font-extrabold text-[#211F1D]">Frequently Asked Questions</h2>
                    <p className="text-[10px] text-[#A8A196] mt-0.5">Common questions about the verification process</p>
                  </div>
                </div>
              ...
              >
                <BadgeCheck className="h-8 w-8 text-[#FF5A36] mx-auto mb-3" />
                <h3 className="text-sm font-extrabold text-[#211F1D] tracking-tight mb-2">
                  Thank you for choosing AnveshakHub
                </h3>
                <p className="text-xs text-[#78716A] leading-relaxed max-w-lg mx-auto">
              ...
              >
                <Link
                  ...
                  className="h-11 px-6 inline-flex items-center justify-center gap-2 bg-[#FF5A36] hover:bg-[#E04826] text-white rounded-xl text-xs font-bold shadow-[var(--shadow-sm)] transition-all hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-[#FF5A36]/30"
                >
                  <Search className="h-4 w-4" /> Track Verification Status
                </Link>
                <Link
                  href="/auth/login"
                  className="h-11 px-6 inline-flex items-center justify-center gap-2 border border-[#E2DCD2] bg-[#FBF7F0] hover:bg-[#FBF7F0] text-[#211F1D] rounded-xl text-xs font-bold shadow-[var(--shadow-sm)] transition-all hover:-translate-y-0.5 hover:shadow-md"
                >
                ...
                <button
                  ...
                  className="h-11 px-6 inline-flex items-center justify-center gap-2 border border-[#E2DCD2] bg-[#FBF7F0] hover:bg-[#FBF7F0] text-[#211F1D] rounded-xl text-xs font-bold shadow-[var(--shadow-sm)] transition-all hover:-translate-y-0.5 hover:shadow-md disabled:opacity-60"
                >
                  {downloading ? (
                    <><Loader2 className="h-4 w-4 animate-spin" /> Generating…</>
                  ) : downloadDone ? (
                    <><CheckCircle2 className="h-4 w-4 text-[#2F6B4F]" /> Downloaded</>
                  ) : (
            ...
            >
              ...
              <div className="card-flat rounded-2xl p-6">
                <p className="text-[10px] font-bold text-[#A8A196] uppercase tracking-widest mb-4 flex items-center gap-1.5">
                ...
                <div className="space-y-2.5">
                  {[
                    ...
                    <div key={label} className="flex justify-between items-start py-1.5 border-b border-slate-50 last:border-0">
                      <span className="text-[10px] font-bold text-[#A8A196] uppercase tracking-wider w-24 shrink-0">{label}</span>
                      <span className="text-[11px] font-bold text-[#211F1D] text-right break-all">{value}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-4 flex items-center gap-2 bg-[#FEF3C7] border border-amber-200 rounded-xl px-3 py-2.5">
                  <Hourglass className="h-4 w-4 text-amber-600 shrink-0" />
                  <p className="text-[10px] font-bold text-[#B45309]">Awaiting compliance review</p>
                </div>
              </div>
              ...
              <div className="card-flat rounded-2xl p-6">
                ...
                <div className="mt-4 pt-3 border-t border-[#E2DCD2]">
                  <a
                    href="#"
                    className="flex items-center gap-1.5 text-xs font-bold text-[#FF5A36] hover:text-[#FF5A36] hover:underline transition-colors"
                  >
              ...
              <div className="card-flat rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-xs font-bold text-[#211F1D]">Security Notice</p>
                </div>
                <p className="text-[10px] text-[#78716A] leading-relaxed">
                  ...
                  All communications will originate from <span className="font-bold text-[#57534E]">@anveshakhub.com</span> domains only.
                </p>
              </div>
            </motion.aside>
          </div>
