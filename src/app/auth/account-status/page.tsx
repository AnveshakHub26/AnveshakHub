...
const lockDetails: Record<LockReason, LockStatus> = {
  failed_logins: {
    ...
    badgeColor: "bg-[#FEE2E2] text-[#C0392B] border-[#FECACA]",
  ...
  temporary_suspension: {
    ...
    badgeColor: "bg-[#FFF4ED] text-[#C2410C] border-orange-200",
  ...
  verification_pending: {
    ...
    badgeColor: "bg-[#FEF3C7] text-[#B45309] border-amber-200",
  ...
  disabled: {
    ...
    badgeColor: "bg-[#EFE9DF] text-[#211F1D] border-slate-350",
          ...
          >
            ...
            <div className="px-6 py-5 border-b border-[#E2DCD2] flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="h-9 w-9 rounded-xl bg-[#FEE2E2] border border-red-100 flex items-center justify-center">
                  <BadgeAlert className="h-[1.125rem] w-[1.125rem] text-[#C0392B]" />
                </div>
                <div>
                  <Dialog.Title className="text-sm font-extrabold text-[#211F1D]">Submit Appeal & Support Request</Dialog.Title>
                  <p id="status-support-desc" className="text-[10px] text-[#A8A196] mt-0.5">AnveshakHub Account Access Resolution Desk</p>
                </div>
              </div>
              ...
              {!sent ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#78716A] uppercase tracking-wider mb-1.5">Official Email <span className="text-red-500">*</span></label>
                      <input
                        required
                        type="email"
                        value={form.email}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-[#78716A] uppercase tracking-wider mb-1.5">Locked Reason Reference</label>
                    <input
                      value={reasonTitle}
                      readOnly
                      className="w-full h-9 px-3 rounded-lg border border-slate-150 bg-[#FBF7F0] text-xs font-bold text-[#57534E] outline-none cursor-default"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-[#78716A] uppercase tracking-wider mb-1.5">Appeal Message <span className="text-red-500">*</span></label>
                    <textarea
                      required
                      rows={4}
                      value={form.message}
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="text-sm font-extrabold text-[#211F1D]">Appeal Submitted</h3>
...
export default function AccountStatusPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ backgroundColor: "var(--bg-app)" }}>
      ...
      <div className="absolute -top-32 right-0 w-[420px] h-[420px] bg-[#FEE2E2]0/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <header className="sticky top-0 z-50 bg-[#FBF7F0]/80 backdrop-blur-md border-b border-[#E2DCD2]/80 py-3 px-6 sm:px-10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="h-8 px-3 inline-flex items-center gap-1.5 rounded-lg border border-[#E2DCD2] bg-[#FBF7F0] text-xs font-bold text-[#57534E] hover:bg-[#FBF7F0] hover:text-[#FF5A36] transition-colors"
          >
      ...
      <main className="flex-grow flex items-center justify-center px-4 sm:px-6 py-12 relative z-10">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 mb-6 justify-center">
            {(["failed_logins", "temporary_suspension", "verification_pending", "disabled", "policy_violation"] as LockReason[]).map(key => (
              <button
                ...
                className={[
                  "px-3 py-1.5 rounded-lg border text-[10px] font-bold transition-all shadow-[var(--shadow-sm)]",
                  activeReason === key
                    ? "bg-[#1C1917] text-white border-slate-800"
                    : "bg-[#FBF7F0] text-[#78716A] border-[#E2DCD2] hover:bg-[#EFE9DF]"
                ].join(" ")}
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-7 items-start">
            ...
            <div className="lg:col-span-7 bg-[#FBF7F0] border border-[#E2DCD2] rounded-2xl shadow-lg overflow-hidden">
              {/* Header Banner */}
              <div className="px-8 pt-8 pb-6 border-b border-[#E2DCD2]">
                <div className="flex items-center gap-3.5 mb-4">
                  <div className="h-10 w-10 rounded-xl bg-[#FEE2E2] border border-red-100 flex items-center justify-center shrink-0">
                    <UserX className="h-5 w-5 text-[#C0392B]" />
                  </div>
                  <div>
                    <span className={[
                      "inline-flex items-center px-2 py-0.5 rounded-full border text-[10px] font-black uppercase tracking-wider mb-1",
                      status.badgeColor
                    ].join(" ")}>
                      {status.statusBadge}
                    </span>
                    <h1 className="text-lg font-black text-[#211F1D] tracking-tight">
                      {status.title}
                    </h1>
                  </div>
                </div>
              ...
              <div className="px-8 py-5 bg-[#FBF7F0] border-b border-[#E2DCD2] space-y-3.5">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] font-bold text-[#A8A196] uppercase tracking-widest">Estimated Unlock Time</p>
                    <p className="text-xs font-extrabold text-[#211F1D] mt-1 flex items-center gap-1.5">
                      <Clock className="h-4 w-4 text-[#A8A196]" />
                      {status.estimatedUnlock ?? "Autounlock unavailable"}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-[#A8A196] uppercase tracking-widest">Policy Category</p>
                    <p className="text-xs font-extrabold text-[#211F1D] mt-1 flex items-center gap-1.5">
                      <ShieldCheck className="h-4 w-4 text-[#A8A196]" />
                      Compliance & Identity
                    </p>
                  </div>
                </div>
              ...
              <div className="px-8 py-7">
                <p className="text-[10px] font-bold text-[#A8A196] uppercase tracking-widest mb-4">
                  Recommended Recovery Instructions
                </p>
                <div className="space-y-3">
                  {status.instructions.map((inst, idx) => (
                    <div key={idx} className="flex gap-3">
                      <div className="h-5 w-5 rounded-full bg-[#EFE9DF] border border-[#E2DCD2] flex items-center justify-center shrink-0 text-[10px] font-black text-[#78716A]">
                        {idx + 1}
                      </div>
                ...
                <div className="mt-8 flex gap-3 flex-wrap">
                  <button
                    onClick={() => setShowSupportModal(true)}
                    className="h-11 px-5 inline-flex items-center gap-2 bg-[#FF5A36] hover:bg-[#E04826] text-white rounded-xl text-xs font-bold shadow-[var(--shadow-sm)] transition-all hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-[#FF5A36]/30"
                  >
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              <div className="card-flat rounded-2xl p-6">
                <div className="flex items-center gap-2.5 mb-4 pb-3 border-b border-[#E2DCD2]">
                  <div className="h-7 w-7 rounded-lg bg-[#FEE2E2] border border-red-100 flex items-center justify-center">
                    <ShieldCheck className="h-4 w-4 text-[#C0392B]" />
                  </div>
                  <p className="text-xs font-extrabold text-[#211F1D]">Compliance Notice</p>
                </div>
                ...
                <div className="mt-4 p-3 bg-[#FEE2E2]/50 border border-red-100 rounded-xl flex gap-2">
                  <AlertTriangle className="h-[1.125rem] w-[1.125rem] text-[#C0392B] shrink-0 mt-0.5" />
              ...
              <div className="card-flat rounded-2xl p-6">
                <div className="flex items-center gap-2.5 mb-4 pb-3 border-b border-[#E2DCD2]">
                  ...
                  <p className="text-xs font-extrabold text-[#211F1D]">Enterprise Helpdesk</p>
                </div>
                  ...
                  {[
                    ...
                    <div key={label} className="flex items-start gap-2.5">
                      <div className="h-6 w-6 rounded bg-[#EFE9DF] flex items-center justify-center shrink-0 mt-0.5">
                        <Icon className="h-3.5 w-3.5 text-[#78716A]" />
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-[#A8A196] uppercase tracking-wider">{label}</p>
                        <p className="text-[11px] font-bold text-[#211F1D] mt-0.5">{value}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="card-flat rounded-2xl p-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#A8A196] mt-0.5">Access Control FAQ</p>
                  </div>
                </div>
                <a
                  href="#"
                  className="h-8 px-3 inline-flex items-center justify-center rounded-lg border border-[#E2DCD2] bg-[#FBF7F0] text-[10px] font-bold text-[#57534E] hover:bg-[#FBF7F0] transition-colors shadow-[var(--shadow-sm)]"
                >
                  View FAQ
                </a>
              </div>
