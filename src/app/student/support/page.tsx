  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Student Support & Help Center</h1>
          <p className="text-xs text-slate-500 mt-0.5">Search FAQs, submit technical helpdesk tickets & track resolution timeline with AnveshakHub support engineers</p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
        <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wide">Search Student Knowledge Base & FAQs</h3>
        <div className="relative">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search internship applications, stipend payouts, document uploads, or meeting scheduling FAQs..."
          ...
          {tickets.map(tkt => (
            <div key={tkt.id} className="border border-slate-100 rounded-xl p-4 flex items-center justify-between bg-slate-50">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link href={`/student/support/${tkt.id}`} className="h-7 px-3 border border-slate-200 hover:border-primary text-slate-600 hover:text-primary rounded-lg text-[10px] font-bold flex items-center gap-1">
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                  <MessageSquare className="h-4 w-4 text-primary" /> Create Student Support Ticket
                </h3>
