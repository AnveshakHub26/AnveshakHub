...
const LIFECYCLE_STYLES: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  ...
  IN_PROGRESS:   { label: "In Progress",   color: "text-[#FF5A36]",    bg: "bg-[#FFF0ED]/70", dot: "bg-[#FF5A36]" },
  ...
  ARCHIVED:      { label: "Archived",      color: "text-[#78716A]",   bg: "bg-[#EFE9DF]",  dot: "bg-[#A8A196]" }
};
    ...
    <div className="flex flex-col min-h-full bg-[#FBF7F0]">
      ...
      <div className="bg-[#FBF7F0] border-b border-[#E2DCD2] px-8 py-5">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-[#211F1D]">Projects Control Center</h1>
            <p className="text-xs text-[#78716A] mt-0.5">Define research scopes, assign subject experts, link student interns, and track lifecycle budgets</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={fetchProjects} className="h-8 px-3 inline-flex items-center gap-1.5 border border-[#E2DCD2] text-[#57534E] rounded-lg text-xs font-medium hover:bg-[#FBF7F0] transition-colors">
            ...
            <button onClick={() => setWizardOpen(true)} className="h-8 px-4 inline-flex items-center gap-1.5 bg-[#FF5A36] text-white rounded-lg text-xs font-bold hover:bg-[#E04826] transition-colors">
        ...
        {stats && (
          <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              { label: "Budget Committed", value: formatCurrency(stats.totalBudget), icon: Wallet, bg: "bg-[#E8F2EC]", color: "text-[#2F6B4F]" },
              { label: "Avg Milestone Completion", value: `${stats.avgProgress}%`, icon: TrendingUp, bg: "bg-[#FF5A36]/50", color: "text-[#FF5A36]/600" },
              { label: "Pipeline Under Review", value: stats.underReview, icon: Clock, bg: "bg-[#FEF3C7]", color: "text-amber-600" },
            ].map((stat) => {
      ...
      <div className="bg-[#FBF7F0] border-b border-[#E2DCD2] px-8 py-3 flex items-center gap-3 flex-wrap">
        ...
        >
          ...
          {lifecycleFilter !== "ALL" && (
            <span className="ml-0.5 w-4 h-4 bg-[#FF5A36] text-white rounded-full text-[10px] flex items-center justify-center font-bold">1</span>
          )}
        </button>

        <span className="text-xs text-[#78716A] ml-auto">{total} active project{total !== 1 ? "s" : ""}</span>
      </div>
        ...
        {showFilters && stats && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-[#FBF7F0] border-b border-[#E2DCD2]">
            <div className="px-8 py-3.5 flex flex-wrap items-center gap-6">
              <div className="flex items-center gap-2">
                <label className="text-[10px] font-semibold text-[#78716A] uppercase tracking-wide">Lifecycle Stage</label>
                <select value={lifecycleFilter} onChange={(e) => setLifecycleFilter(e.target.value)} className="h-7 text-xs border border-[#E2DCD2] rounded-lg px-2 bg-[#FBF7F0] focus:outline-none focus:border-[#FF5A36]">
                  <option value="ALL">All Stages</option>
                  {Object.entries(LIFECYCLE_STYLES).map(([key, style]) => <option key={key} value={key}>{style.label}</option>)}
                </select>
              </div>
              {lifecycleFilter !== "ALL" && (
                <button onClick={() => setLifecycleFilter("ALL")} className="text-xs text-[#FF5A36] hover:underline font-semibold">Reset Filters</button>
              )}
            </div>
          </motion.div>
        )}
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-bold text-[#A8A196] uppercase tracking-wider">{p.id}</span>
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-[#211F1D] leading-snug h-8 line-clamp-2">{p.name}</h3>
                  ...
                  <div className="flex justify-between text-[10px] text-[#78716A] font-medium">
                    <span>Budget Allocation</span>
                    <span className="font-bold text-[#211F1D]">{formatCurrency(p.budget)}</span>
                  </div>

                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-[#78716A] font-bold uppercase">
                  ...
                  <div className="flex items-center justify-between text-[10px] text-[#A8A196] pt-2 border-t border-slate-50">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setWizardOpen(false)}>
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="bg-[#FBF7F0] rounded-2xl shadow-2xl p-6 w-[450px] max-w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b border-[#E2DCD2] pb-3 mb-4">
                <h3 className="text-sm font-bold text-[#211F1D]">Launch Project Wizard (Step {wizardStep}/3)</h3>
                <button onClick={() => setWizardOpen(false)} className="text-[#A8A196] hover:text-[#57534E]"><X className="h-4 w-4" /></button>
              </div>

              {wizardStep === 1 && (
                <div className="space-y-3">
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase tracking-wide block mb-1">Project Contract Name *</label>
                    <input value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} placeholder="e.g. Solid-State Battery R&D Hub" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg text-xs" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase tracking-wide block mb-1">Brief Description *</label>
                    <textarea value={newProjectDesc} onChange={(e) => setNewProjectDesc(e.target.value)} rows={3} placeholder="Provide details on project deliverables…" className="w-full p-2.5 border border-[#E2DCD2] rounded-lg text-xs" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase tracking-wide block mb-1">Budget Allocation (INR) *</label>
                    <input type="number" value={newProjectBudget} onChange={(e) => setNewProjectBudget(e.target.value)} placeholder="e.g. 3500000" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg text-xs" />
                  </div>
                </div>
              )}
              ...
              {wizardStep === 2 && (
                <div className="space-y-3">
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase tracking-wide block mb-1">Client Organization (Industry) *</label>
                    <input value={newProjectClient} onChange={(e) => setNewProjectClient(e.target.value)} placeholder="e.g. Solaris Power Pvt Ltd" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg text-xs" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase tracking-wide block mb-1">Subject Expert Lead *</label>
                    <input value={newProjectExpert} onChange={(e) => setNewProjectExpert(e.target.value)} placeholder="e.g. Dr. Arunima Krishnan" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg text-xs" />
                  </div>
                </div>
              )}
              ...
              {wizardStep === 3 && (
                <div className="space-y-3">
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase tracking-wide block mb-1">Problem Statement Summary *</label>
                    <textarea value={newProjectProblem} onChange={(e) => setNewProjectProblem(e.target.value)} rows={2} placeholder="Detail the core issue to solve…" className="w-full p-2.5 border border-[#E2DCD2] rounded-lg text-xs" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase tracking-wide block mb-1">Scope Definition Boundary *</label>
                    <textarea value={newProjectScope} onChange={(e) => setNewProjectScope(e.target.value)} rows={2} placeholder="Outline core deliverables or boundaries…" className="w-full p-2.5 border border-[#E2DCD2] rounded-lg text-xs" />
                  </div>
                </div>
              )}

              <div className="flex gap-2 justify-end border-t border-[#E2DCD2] pt-3 mt-4">
                {wizardStep > 1 && (
                  <button onClick={() => setWizardStep(wizardStep - 1)} className="h-8 px-3 border border-[#E2DCD2] rounded-lg text-xs font-semibold text-[#78716A] hover:bg-[#EFE9DF]">Back</button>
                )}
                {wizardStep < 3 ? (
                  <button onClick={() => setWizardStep(wizardStep + 1)} className="h-8 px-4 bg-[#FF5A36] hover:bg-[#E04826] text-white rounded-lg text-xs font-bold">Continue</button>
                ) : (
