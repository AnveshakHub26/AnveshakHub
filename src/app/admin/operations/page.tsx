    ...
    <div className="flex flex-col min-h-full bg-[#FBF7F0]">
      ...
      <div className="bg-[#FBF7F0] border-b border-[#E2DCD2] px-8 py-5">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-[#211F1D]">Ecosystem Operations Control</h1>
            <p className="text-xs text-[#78716A] mt-0.5">Audit workflow SLA compliance, resolve escalated incidents, and configure process automations</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={fetchOperations} className="h-8 px-3 inline-flex items-center gap-1.5 border border-[#E2DCD2] text-[#57534E] rounded-lg text-xs font-medium hover:bg-[#FBF7F0] transition-colors">
        ...
        {stats && (
          <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: "Active SLA Pipelines", value: stats.activeWorkflows, icon: HardHat, bg: "bg-[#FFF0ED]", color: "text-[#FF5A36]" },
              { label: "Escalated Incidents", value: stats.activeIncidents, icon: ShieldAlert, bg: "bg-[#FEE2E2]", color: "text-red-500" },
              { label: "SLA Compliance Rate", value: stats.slaCompliance, icon: CheckCircle2, bg: "bg-[#E8F2EC]", color: "text-[#2F6B4F]" },
          ...
          <AnimatePresence mode="wait">
                ...
                {workflows.map((wf) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <span className="text-[10px] font-bold text-[#A8A196] uppercase tracking-wider">{wf.id}</span>
                      <span className="text-[10px] px-2 py-0.5 bg-[#FFF0ED] text-[#FF5A36] border border-blue-100 rounded font-bold">{wf.slaHours} Hours SLA</span>
                    </div>

                    <div>
                      <h3 className="text-xs font-bold text-[#211F1D] leading-snug">{wf.name}</h3>
                      <p className="text-[10px] text-[#78716A] mt-1 leading-relaxed">"{wf.description}"</p>
                    </div>

                    <div className="space-y-1.5 pt-2.5 border-t border-slate-50">
                      <span className="text-[10px] text-[#A8A196] uppercase font-bold">Process Milestones:</span>
                      <div className="flex items-center gap-1 flex-wrap text-[10px] text-[#78716A] font-bold">
                        {wf.steps.map((st, idx) => (
                          <React.Fragment key={st}>
                            <span className="px-1.5 py-0.5 bg-[#EFE9DF] rounded">{st}</span>
              ...
              <div className="card-flat rounded-2xl overflow-hidden">
                <table className="w-full border-collapse text-left text-xs text-[#211F1D]">
                  ...
                  <tbody className="divide-y font-medium">
                    {incidents.map((inc) => (
                      <tr key={inc.id} className="hover:bg-[#EFE9DF]/50">
                        <td className="px-6 py-4">
                          <div className="font-bold text-[#211F1D]">{inc.title}</div>
                          <div className="text-[10px] text-[#A8A196] mt-0.5 font-semibold">"{inc.description}"</div>
                        </td>
                        <td className="px-6 py-4 text-slate-550">{inc.workflowName}</td>
                        <td className="px-6 py-4 text-center">
                          <span className={`text-[8px] px-2 py-0.5 rounded font-bold ${inc.severity === "CRITICAL" ? "bg-[#FEE2E2] text-[#C0392B] border border-red-100" : "bg-[#FEF3C7] text-[#B45309] border border-amber-100"}`}>{inc.severity}</span>
                        </td>
                        <td className="px-6 py-4 text-center">
                          <span className="text-[10px] bg-[#FEE2E2] text-[#C0392B] rounded font-bold px-2 py-0.5 border border-red-100">{inc.status}</span>
                        </td>
            ...
            {activeTab === "processes" && (
              <div className="card-flat rounded-2xl p-5 space-y-4">
                <h3 className="text-xs font-bold text-[#211F1D] uppercase tracking-wide flex items-center gap-1.5"><ShieldCheck className="h-[1.125rem] w-[1.125rem] text-[#FF5A36]" /> Process Automation Rules</h3>
                <p className="text-xs text-[#78716A] leading-relaxed font-medium">Automatically dispatch warning reminders to Outreach Coordinators when student internship agreements fail signatures within 72 hours of project approval.</p>
                <button className="h-8 px-4 bg-[#0F0E0C] text-white rounded-lg text-xs font-bold hover:bg-black transition-colors">Run Automation Sweep</button>
              </div>
            )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setCreateOpen(false)}>
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="bg-[#FBF7F0] rounded-2xl shadow-2xl p-6 w-[450px] max-w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-3.5 text-xs">
                <div>
                  <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Workflow Title *</label>
                  <input value={wfName} onChange={(e) => setWfName(e.target.value)} placeholder="e.g. CSR Innovation Funding Milestone SLA" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Description *</label>
                  <textarea value={wfDesc} onChange={(e) => setWfDesc(e.target.value)} rows={3} placeholder="Outline workflow scope limits and timelines…" className="w-full p-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">SLA Hours *</label>
                    <input type="number" value={wfSla} onChange={(e) => setWfSla(e.target.value)} placeholder="e.g. 24" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Workflow Steps (comma separated) *</label>
                    <input value={wfSteps} onChange={(e) => setWfSteps(e.target.value)} placeholder="Step 1, Step 2, Step 3" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                  </div>
                </div>
              </div>

              <div className="flex justify-end gap-2 border-t border-[#E2DCD2] pt-3.5 mt-4">
                <button onClick={handleCreateWorkflow} disabled={createLoading} className="h-8 px-4 bg-[#FF5A36] text-white rounded-lg font-bold flex items-center gap-1 hover:bg-[#E04826]">
