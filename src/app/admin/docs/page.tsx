...
export default function DevDocsPage() {
  return (
    <div className="p-6 space-y-6 flex-grow relative z-10 flex flex-col max-w-4xl mx-auto">
      <div>
        <h1 className="text-xl font-black text-[#211F1D] tracking-tight">System Developer Documentation</h1>
        <p className="text-xs text-[#78716A] mt-0.5">Technical specifications, API catalogs, Prisma schemas, and directory structure guidelines.</p>
      </div>

      <div className="space-y-6">
        
        {/* Directory Structure */}
        <section className="card-flat rounded-2xl p-6 shadow-[var(--shadow-sm)]">
          <h2 className="text-xs font-extrabold text-[#211F1D] uppercase tracking-wider mb-4 flex items-center gap-2">
            <Layers className="h-[1.125rem] w-[1.125rem] text-[#FF5A36]" />
            1. System Folder Architecture
          </h2>
          <pre className="bg-[#0F0E0C] text-slate-200 rounded-xl p-5 text-[11px] font-mono leading-relaxed overflow-x-auto select-all">
{`d:/AnveshakHub
├── prisma/
...
│       └── footer.tsx                # Reusable system footer`}
        ...
        <section className="card-flat rounded-2xl p-6 shadow-[var(--shadow-sm)]">
          <h2 className="text-xs font-extrabold text-[#211F1D] uppercase tracking-wider mb-4 flex items-center gap-2">
            <Server className="h-[1.125rem] w-[1.125rem] text-[#FF5A36]" />
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={api.url} className="border border-[#E2DCD2] rounded-xl p-4 bg-[#FBF7F0] space-y-2">
                <div className="flex items-center gap-2.5">
                  <span className={[
                    "text-[10px] font-black px-2 py-0.5 rounded border leading-none shrink-0",
                    api.method === "GET" ? "bg-[#E8F2EC] text-[#2F6B4F] border-[#BBD9C8]" : "bg-[#FFF0ED] text-[#FF5A36] border-[#FFCFC4]"
                  ].join(" ")}>{api.method}</span>
                  <code className="text-xs font-mono font-bold text-[#211F1D]">{api.url}</code>
                </div>
                <p className="text-[11px] text-[#78716A] leading-relaxed font-medium">{api.desc}</p>
                <pre className="bg-[#0F0E0C] text-[#D8D2C7] rounded-lg p-3 text-[10px] font-mono leading-relaxed overflow-x-auto select-all">
                  {api.curl}
                </pre>
              </div>
            ))}
        ...
        <section className="card-flat rounded-2xl p-6 shadow-[var(--shadow-sm)]">
          <h2 className="text-xs font-extrabold text-[#211F1D] uppercase tracking-wider mb-4 flex items-center gap-2">
            <ShieldCheck className="h-[1.125rem] w-[1.125rem] text-[#FF5A36]" />
            3. Security, RBAC & Data Integrity Architecture
          </h2>
          <ul className="space-y-3 text-xs text-[#57534E] leading-relaxed">
            <li>
        ...
        <section className="card-flat rounded-2xl p-6 shadow-[var(--shadow-sm)]">
          <h2 className="text-xs font-extrabold text-[#211F1D] uppercase tracking-wider mb-4 flex items-center gap-2">
            <Code className="h-[1.125rem] w-[1.125rem] text-[#FF5A36]" />
            4. Testing Suite Guide
          </h2>
          <p className="text-xs text-[#78716A] mb-3.5">Verify compilation and pipeline state logic using the following terminal command protocols:</p>
          <pre className="bg-[#0F0E0C] text-slate-200 rounded-xl p-4 text-[10px] font-mono leading-relaxed overflow-x-auto select-all">
