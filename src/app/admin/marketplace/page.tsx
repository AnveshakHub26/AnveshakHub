    ...
    <div className="flex flex-col min-h-full bg-[#FBF7F0]">
      ...
      <div className="bg-[#FBF7F0] border-b border-[#E2DCD2] px-8 py-5">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-[#211F1D]">B2B Expert Marketplace</h1>
            <p className="text-xs text-[#78716A] mt-0.5">Explore open research calls, send matching recommendations, and invite verified subject experts</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={fetchMarketplace} className="h-8 px-3 inline-flex items-center gap-1.5 border border-[#E2DCD2] text-[#57534E] rounded-lg text-xs font-medium hover:bg-[#FBF7F0] transition-colors">
        ...
        <div className="flex items-center gap-0 mt-5 border-t border-[#E2DCD2] pt-0 -mb-5">
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex items-center gap-1.5 px-4 py-3.5 text-xs font-semibold border-b-2 transition-all ${activeTab === tab.key ? "border-[#FF5A36] text-[#FF5A36]" : "border-transparent text-[#78716A] hover:text-[#211F1D]"}`}>
              {tab.label} <span className="text-[10px] px-1.5 py-0.2 bg-[#EFE9DF] text-[#78716A] rounded-full font-bold">{tab.count}</span>
            </button>
          ))}
        </div>
      </div>
          ...
          <AnimatePresence mode="wait">
                ...
                {opportunities.map((opp) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <span className="text-[10px] font-bold text-[#A8A196] uppercase tracking-wider">{opp.id}</span>
                      <span className="text-[10px] px-2 py-0.5 bg-[#FFF0ED] text-[#FF5A36] rounded font-bold">{opp.domain}</span>
                    </div>

                    <div>
                      <h3 className="text-xs font-bold text-[#211F1D] leading-snug h-8 line-clamp-2">{opp.title}</h3>
                      <p className="text-[10px] text-[#A8A196] font-semibold mt-1 flex items-center gap-1">
                        <Building2 className="h-3 w-3" /> {opp.industry.orgName} · <span className="text-[#2F6B4F] font-bold">{opp.industry.orgType}</span>
                      </p>
                    </div>

                    <p className="text-[10px] text-[#78716A] leading-relaxed line-clamp-2 h-7 font-medium">"{opp.description}"</p>

                    <div className="flex flex-wrap gap-1 mt-1">
                      {opp.requirements.map(r => (
                        <span key={r} className="text-[8px] bg-[#FBF7F0] border border-[#E2DCD2] text-[#57534E] px-1.5 py-0.2 rounded font-semibold">{r}</span>
                      ))}
                    </div>

                    <div className="flex justify-between items-center text-[10px] text-[#78716A] pt-2.5 border-t border-slate-50">
                      <div>
                        <span className="text-[10px] text-[#A8A196] uppercase block font-semibold leading-none">Consulting Budget</span>
                        <span className="font-extrabold text-slate-850 text-xs">{formatCurrency(opp.budget)}</span>
                      </div>
                      
                      <div className="flex gap-1.5">
                        <button onClick={() => toggleBookmark(opp.id)} className="h-8 w-8 border border-[#E2DCD2] text-[#78716A] rounded-xl hover:bg-[#FBF7F0] flex items-center justify-center">
                        ...
                        <button
                          ...
                          className="h-8 px-3.5 bg-[#FF5A36] hover:bg-[#E04826] text-white rounded-xl text-[10px] font-bold transition-all flex items-center gap-1"
                        >
            ...
            {activeTab === "recommendations" && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} className="space-y-4">
                <div className="card-flat rounded-2xl p-5 space-y-2">
                  <h3 className="text-xs font-bold text-[#211F1D] uppercase tracking-wide flex items-center gap-1.5"><Zap className="h-[1.125rem] w-[1.125rem] text-[#FF5A36] fill-current" /> compliance matching telemetry</h3>
                  <p className="text-xs text-[#78716A] font-medium">Verified recommendations linking featured experts and current consulting bids</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {recommendations.map((rec) => (
                    <div key={rec.expertId} className="card-flat rounded-2xl p-5 flex flex-col justify-between gap-3">
                      <div>
                        <div className="flex justify-between items-start">
                          <h4 className="text-xs font-bold text-[#211F1D]">{rec.name}</h4>
                          <span className="text-[10px] px-2 py-0.5 bg-[#E8F2EC] text-[#2F6B4F] border border-green-150 rounded-full font-bold">{rec.matchScore}% Match Score</span>
                        </div>
                        <p className="text-[10px] text-[#78716A] italic mt-2">"{rec.reason}"</p>
                        
                        <div className="flex flex-wrap gap-1 mt-3">
                          {rec.skills.map(s => (
                            <span key={s} className="text-[8px] bg-[#FFF0ED]/50 border border-blue-100 text-[#FF5A36] px-2 py-0.5 rounded font-semibold">{s}</span>
                          ))}
                        </div>
                      </div>

                      <div className="pt-3 border-t border-slate-50 flex gap-2 justify-end">
                        <button className="h-8 px-4 bg-[#FF5A36] hover:bg-[#E04826] text-white rounded-xl text-[10px] font-bold">Direct Invite</button>
                        <button className="h-8 px-3 border border-[#E2DCD2] text-[#57534E] hover:bg-[#FBF7F0] rounded-xl text-[10px] font-semibold">View Profile</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {activeTab === "reviews" && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-[#FBF7F0] border border-[#E2DCD2] rounded-2xl p-5 space-y-4">
                  <h3 className="text-xs font-bold text-[#211F1D] uppercase tracking-wide">Client Testimonials & Ratings</h3>
                  
                  <div className="divide-y">
                    {reviews.map((rev) => (
                      <div key={rev.id} className="py-4 first:pt-0 last:pb-0 space-y-1.5">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-bold text-[#211F1D]">{rev.reviewerName}</span>
                          <div className="flex gap-0.5 text-amber-400">
                            {Array.from({ length: rev.rating }).map((_, i) => <Star key={i} className="h-3 w-3 fill-current" />)}
                          </div>
                        </div>
                        <p className="text-[11px] text-[#78716A] italic leading-relaxed">"{rev.comment}"</p>
                        <p className="text-[10px] text-[#A8A196] font-semibold">{new Date(rev.createdAt).toLocaleDateString("en-IN")}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="card-flat rounded-2xl p-5 space-y-4">
                  <h3 className="text-xs font-bold text-[#211F1D] uppercase tracking-wide flex items-center gap-1.5"><ShieldCheck className="h-[1.125rem] w-[1.125rem] text-[#FF5A36]" /> Compliance Moderation</h3>
                  
                  <div className="space-y-3 text-xs">
                    <div className="p-3 bg-[#FBF7F0] border border-[#E2DCD2] rounded-xl space-y-1">
                      <div className="font-semibold text-[#211F1D]">DPIIT Audit Check</div>
                      <p className="text-[10px] text-[#78716A]">Ensure consultings match the legal limits of government R&D grants.</p>
                    </div>

                    <button className="w-full h-8 bg-[#0F0E0C] text-white rounded-lg font-bold text-xs hover:bg-black transition-colors">Run Audit Check</button>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setProposalModalOpen(false)}>
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="bg-[#FBF7F0] rounded-2xl shadow-2xl p-6 w-[450px] max-w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b border-[#E2DCD2] pb-3 mb-4">
                ...
                <button onClick={() => setProposalModalOpen(false)} className="text-[#A8A196] hover:text-[#57534E]"><X className="h-4 w-4" /></button>
              </div>

              <div className="space-y-3 text-xs">
                <p className="text-[#78716A] leading-normal">You are applying as a consultant for this project. Outline your proposal, methodology, and availability checks.</p>
                
                <div>
                  <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Proposal & Scope Details *</label>
                  <textarea value={proposalText} onChange={(e) => setProposalText(e.target.value)} rows={5} placeholder="Describe how your expertise connects with the requirements…" className="w-full p-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                </div>
              </div>

              <div className="flex justify-end gap-2 border-t border-[#E2DCD2] pt-3.5 mt-4">
                <button onClick={handleApplyOpportunity} disabled={proposalLoading} className="h-8 px-4 bg-[#FF5A36] text-white rounded-lg font-bold flex items-center gap-1 hover:bg-[#E04826]">
                  {proposalLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null} Submit Proposal
                </button>
                <button onClick={() => setProposalModalOpen(false)} className="h-8 px-3 border border-[#E2DCD2] text-[#78716A] rounded-lg font-semibold hover:bg-[#EFE9DF]">Cancel</button>
              </div>
            </motion.div>
          </motion.div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setPublishOpen(false)}>
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="bg-[#FBF7F0] rounded-2xl shadow-2xl p-6 w-[450px] max-w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-3.5 text-xs">
                <div>
                  <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Opportunity Title *</label>
                  <input value={pubTitle} onChange={(e) => setPubTitle(e.target.value)} placeholder="e.g. DNA Sequencing AI Architect" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Detailed Description *</label>
                  <textarea value={pubDesc} onChange={(e) => setPubDesc(e.target.value)} rows={3} placeholder="Outline research consulting expectations…" className="w-full p-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Research Domain *</label>
                    <select value={pubDomain} onChange={(e) => setPubDomain(e.target.value)} className="w-full h-8 px-2 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]">
                      <option value="AI & Smart Grids">AI & Smart Grids</option>
                      <option value="BioTech">BioTech</option>
                      <option value="Aerospace">Aerospace</option>
                    </select>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Consulting Budget (INR) *</label>
                    <input type="number" value={pubBudget} onChange={(e) => setPubBudget(e.target.value)} placeholder="e.g. 500000" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                  </div>
                </div>

                <div>
                  <label className="text-[10px] font-bold text-[#78716A] uppercase block mb-1">Key Requirements (comma separated)</label>
                  <input value={pubReqs} onChange={(e) => setPubReqs(e.target.value)} placeholder="e.g. Ph.D, 5+ Yrs Exp, PyTorch" className="w-full h-8 px-2.5 border border-[#E2DCD2] rounded-lg bg-[#FBF7F0]" />
                </div>
              </div>

              <div className="flex justify-end gap-2 border-t border-[#E2DCD2] pt-3.5 mt-4">
                <button onClick={handlePublishOpportunity} disabled={pubLoading} className="h-8 px-4 bg-[#FF5A36] text-white rounded-lg font-bold flex items-center gap-1 hover:bg-[#E04826]">
