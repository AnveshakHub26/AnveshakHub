    ...
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-[#E2DCD2] pb-4">
        <div className="flex items-center gap-3">
          <Link href="/industry/problem-statements" className="h-8 w-8 rounded-lg border border-[#E2DCD2] flex items-center justify-center text-[#78716A] hover:text-[#211F1D] hover:bg-[#FBF7F0] transition-colors">
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-bold text-[#A8A196] uppercase tracking-wide">{problem.category}</span>
              <span className="text-[#C4BCB0]">·</span>
              <span className="text-[9px] font-semibold text-[#A8A196]">Version {problem.version}.0</span>
            </div>
            <h1 className="text-sm font-bold text-[#211F1D] mt-0.5">{problem.title}</h1>
          </div>
        </div>
      ...
      {/* Workflow Timeline */}
      <div className="bg-white border border-[#E2DCD2] rounded-2xl p-5 flex items-center justify-around text-center">
        {[
            ...
            <div key={step.key} className="flex-1 relative flex flex-col items-center">
              <div className={`h-8 w-8 rounded-full flex items-center justify-center border-2 transition-all ${
                isActive
                  ...
                  : "bg-white border-[#E2DCD2] text-[#A8A196]"
              }`}>
                {isPassed && !isActive ? <CheckCircle2 className="h-4 w-4" /> : <span className="text-[10px] font-bold">{idx + 1}</span>}
              </div>
              <p className="text-[10px] font-bold text-[#211F1D] mt-2">{step.label}</p>
              <p className="text-[8px] text-[#A8A196] font-semibold">{step.desc}</p>
            </div>
          );
        })}
      </div>
          ...
          {activeTab === "overview" && (
            <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs">
              <div className="lg:col-span-2 space-y-4">
                <div>
                  <h3 className="text-[10px] font-bold text-[#A8A196] uppercase tracking-wide">Problem Scope & Objective</h3>
                ...
                {problem.aiAnalysis ? (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-[#57534E]">Feasibility Match Score</span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-2.5 bg-white border border-[#E2DCD2] rounded-xl">
                        <span className="text-[8px] text-[#A8A196] font-bold block uppercase">Difficulty</span>
                        <span className="text-[10px] font-bold text-[#211F1D]">{problem.aiAnalysis.difficulty}</span>
                      </div>
                      <div className="p-2.5 bg-white border border-[#E2DCD2] rounded-xl">
                        <span className="text-[8px] text-[#A8A196] font-bold block uppercase">Est. Development</span>
                        <span className="text-[10px] font-bold text-[#211F1D]">{problem.aiAnalysis.estimatedDevelopmentDays} Days</span>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[8px] text-[#A8A196] font-bold block uppercase">Recommended Keywords</span>
                    ...
                    <div className="space-y-1">
                      <span className="text-[8px] text-[#A8A196] font-bold block uppercase">Key Technical Obstacles</span>
                      <ul className="list-disc pl-4 space-y-1 text-[9px] text-[#78716A] font-semibold leading-relaxed">
                  ...
                  <p className="text-[10px] text-[#A8A196] leading-relaxed">
          ...
          {activeTab === "documents" && (
            <motion.div key="documents" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 text-xs">
              <div className="flex items-center justify-between border-b border-[#E2DCD2] pb-3">
                <h3 className="text-xs font-bold text-[#211F1D] uppercase tracking-wide">Supporting Attachments</h3>
                  ...
                  <div key={d.id} className="border border-[#E2DCD2] rounded-xl p-3 flex items-center justify-between hover:border-[#E2DCD2] transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[9px] text-[#A8A196] font-semibold mt-0.5">{(d.fileSize ? d.fileSize / 1024 : 0).toFixed(0)} KB · By {d.uploadedBy}</p>
                      </div>
                    </div>
            ...
            <motion.div key="comments" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 text-xs">
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {problem.comments.map(c => (
                  <div key={c.id} className="bg-[#FBF7F0] border border-[#E2DCD2] rounded-xl p-3.5 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-[#211F1D] flex items-center gap-1">
                        <User className="h-3.5 w-3.5 text-[#A8A196]" /> {c.authorName}
                      </span>
                      <span className="text-[8px] text-[#A8A196] font-semibold">{new Date(c.createdAt).toLocaleString("en-IN")}</span>
                    </div>
                    <p className="text-[#57534E] leading-relaxed font-semibold">{c.content}</p>
                  </div>
                ))}
              </div>
              <div className="border-t border-[#E2DCD2] pt-4 flex gap-3">
                <input
                  value={newComment}
            ...
            <motion.div key="timeline" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative border-l border-[#E2DCD2] pl-4 ml-2 space-y-4 text-xs">
              {problem.timeline.map(event => (
                <div key={event.id} className="relative">
                  ...
                  <p className="text-[10px] text-[#78716A] leading-relaxed mt-0.5">{event.description}</p>
                  <span className="text-[8px] text-[#A8A196] font-semibold mt-0.5 block">{new Date(event.createdAt).toLocaleString("en-IN")} · Performed by {event.performedBy}</span>
                </div>
              ))}
            </motion.div>
          )}
